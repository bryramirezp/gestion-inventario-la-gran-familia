...
const Sidebar: React.FC<{
...
}> = ({ isCollapsed, isMobileOpen, setMobileOpen }) => {
  ...
  const sidebarContent = (
    <div className={`flex flex-col h-full ${isCollapsed ? 'lg:hidden' : ''}`}>
      <div
        className={`flex items-center mb-6 px-2 ${isCollapsed ? 'justify-center' : 'justify-start'}`}
      >
        <div
          className={`flex items-center overflow-hidden transition-all duration-300 ${isCollapsed ? 'w-0' : 'w-auto'}`}
        >
      ...
      <nav
        className={`flex-1 space-y-2 overflow-y-auto overflow-x-hidden -mr-2 pr-2 ${isCollapsed ? 'lg:hidden' : ''}`}
      >
        {navigationSections.map((section) => {
          ...
          return (
            <div key={section.title}>
              <h3
                className={`px-3 text-xs font-semibold uppercase text-muted-foreground/80 tracking-wider mb-1 mt-3 ${isCollapsed ? 'text-center' : ''}`}
              >
                <span className={isCollapsed ? 'hidden lg:inline-block' : ''}>{section.title}</span>
              </h3>
              {availableItems.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `${linkClasses} ${isActive ? activeLinkClasses : ''} ${isCollapsed ? 'justify-center' : ''} group relative`
                  }
                  onMouseEnter={() => {
                ...
                >
                  ...
                  <span
                    className={`ml-3 whitespace-nowrap transition-opacity duration-200 ${isCollapsed ? 'lg:hidden' : ''}`}
                  >
                    {item.name}
                  </span>
                  <span
                    className={`
                                    ...
                                    transition-all scale-95 opacity-0 group-hover:scale-100 group-hover:opacity-100
                                    ${!isCollapsed ? 'hidden' : ''}
                                `}
                  >
                    {item.name}
      ...
      <div className={`mt-auto flex-shrink-0 ${isCollapsed ? 'lg:hidden' : ''}`}>
        {user && (
          <div className="border-t border-border dark:border-dark-border pt-4">
            ...
            >
              ...
              <span className={`ml-2 whitespace-nowrap ${isCollapsed ? 'lg:hidden' : ''}`}>
                Cerrar Sesión
              </span>
            </Button>
          </div>
    ...
    <>
      ...
      <aside
        className={`
            ...
            w-64
            ${isCollapsed ? 'lg:w-0 lg:border-r-0 lg:p-0' : 'lg:w-64'}
